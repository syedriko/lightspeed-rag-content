...
# Regenerate rpms.lock.yaml (needs Red Hat activation key + org id).
# Optional: REGISTRY_USERNAME / REGISTRY_PASSWORD if BASE_IMAGE needs auth.
update-rpm-lock:
	@echo "Run from repo root: ./scripts/generate-rpm-lock.sh -a '<activation-key>' -g '<org-id>' [-f rpms.in.yaml] [-O rpms.lock.yaml]"
	@echo "See ./scripts/generate-rpm-lock.sh -h for options."
